...
class BahanATKController extends Controller
{
    ...
    public function insertData(InsertRequest $request)
    {
        ...
        try {
            ...
            if ($insertResult == false) {
                return $this->responseError('Gagal menyimpan data bahan ATK', 500);
            }

            DB::commit();
            return $this->responseSuccess('Data bahan ATK berhasil disimpan', 200, ['bahan_baku_id' => $autonumber]);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function updateData(UpdateRequest $request)
    {
        ...
        try {
            ...
            if ($updateResult == false) {
                return $this->responseError('Gagal memperbarui data bahan ATK', 500);
            }

            DB::commit();
            return $this->responseSuccess('Data bahan ATK berhasil diperbarui', 200, ['bahan_baku_id' => $request->bahan_baku_id]);
        } catch (\Exception $e) {
            DB::rollBack();
            return $this->responseError('Terjadi kesalahan: ' . $e->getMessage(), 500);
        }
    ...
    public function deleteData(DeleteRequest $request)
    {
        ...
        if ($cek == false) {
         ...
         if ($cek != false) {
             return $this->responseError('Bahan ATK sedang digunakan', 400);
         }
        ...
        try {
            ...
            if ($deleteResult == false) {
                return $this->responseError('Gagal menghapus data bahan ATK', 500);
            }

            DB::commit();
            return $this->responseSuccess('Data bahan ATK berhasil dihapus', 200, ['bahan_baku_id' => $id]);
